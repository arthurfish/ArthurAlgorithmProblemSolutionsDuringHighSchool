...
#include<cstdio>
#include<cstring>
char key[1024], after[1024], lowerMarker[1024], source[1024];
int now, keyLen, afterLen;

void toNumber(char *a){
    int len = strlen(a);
    for(int i = 0; i < len; i++)
	if('a' <= a[i] && a[i] <= 'z')
	    a[i] = a[i] - 'a';
...
void saveLower(char *a){
    int len = strlen(a);
    for(int i = 0; i < len; i++)
	lowerMarker[i] = ('a' <= a[i] && a[i] <= 'z')?1:0;
}
...
scanf("%s%s", key, after);
	...
	source[i] = after[i]  - key[getNext()];
	if(source[i] < 0)
	    source[i] += 26;
	printf("%c", (lowerMarker[i] == 1)?source[i] + 'a':source[i] + 'A');
    }
    return 0;
}
